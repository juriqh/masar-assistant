...
def get_classes_for_day(user_id: str, day_token: str) -> List[Dict[str, Any]]:
    ...
    res = (
        ...
        .select("*")
        .eq("user_id", user_id)
        .contains("days_of_week", [day_token])
        .eq("active", True)
        .order("start_time")
        .execute()
    )
...
def already_sent(task: str, on_day: date) -> bool:
    ...
    if not sb:
        return False
    start = datetime(on_day.year, on_day.month, on_day.day).isoformat()
    end = datetime(on_day.year, on_day.month, on_day.day, 0, 0) + timedelta(days=1)
    res = (
        sb.table("events_log")
        .select("id")
        .eq("task", task)
